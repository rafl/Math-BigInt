...
sub badd 
  ...
  if ($sx eq $sy)  
    {
    $CALC->_add($x->{value},$y->{value});	# if same sign, absolute add
    $x->{sign} = $sx;
    }
  else 
    {
    ...
    if ($a > 0)                           
      ...
      $CALC->_sub($y->{value},$x->{value},1);		# abs sub w/ swapped
      $x->{sign} = $sy;
      } 
    elsif ($a == 0)
      {
      ...
      $CALC->_sub($x->{value}, $y->{value});		# absolute sub
      $x->{sign} = $sx;
      }
    }
  return $x->round(@bn);
...
sub _split
  ...
  # strip white space at front, also superflous leading zeros
